...
\end{aligned}
\]
The coefficient of 15 is -20 Therefore, the Inverses would be $43 - 20 = 23$. So $23 \cdot 15 \equiv 1 \pmod{43}$.
...
Meaning Inverses of $15 \text{ mod } 43$ is 23.
...
\begin{proof}
    ...
    \[p \mid (x - 1)(x + 1)\]
    Since $p$ is prime (by Lemma 9.4.2) $p \mid (x - 1)$ and $p \mid (x + 1)$ so by defintion of congruence $x \equiv_n -1$ and $x \equiv_n 1$. 
\end{proof}
\pagebreak
...
\begin{proof}[Proof by contradiction]
    ...
    ,So at least one of the condition from the algorithm must hold.
    \begin{itemize}
        \item $x_e \not\equiv_n 1$
    ...
    First condition led to contradiction, So second condition must be true. 
    So $x_{i+1} = {x_i}^2 \equiv_n 1$ but $x_i \not\equiv_n \pm 1$.
    Since n is prime, by lemme from part (b),given condition yields contradiction.    
    
    Both of the condition lead to contradiction meaning our assumption must be wrong so n must indeed be composite.
\end{proof}
...
By the defintion of transition in given state machine we have $x' = y$, $y' = (x \text{ rem } y)$, $s' = u$, $t' = v$, $u' = s - qu$ and $v' = t - qv$ where $x = qy + (x \text{ rem } y)$.
\begin{itemize}
